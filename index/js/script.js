...
(function($, window) {
    ...
    $(document).ready(function() {
        ...
        function makeAjaxSearch(result) {
            if (result.length == 0) {
                $("#search_filtered").empty().show().append('<li><a href="javascript:;"><strong>很抱歉，没有找到你想搜索的东西哦</strong></a></li>');
            } else {
                $("#search_filtered").empty().show();
                for (var i = 0; i < result.length; i++) $("#search_filtered").append('<li><a href="'+result[i]["url"]+'" title="'+result[i]["desc"]+'">' + result[i]["art_title"] + '</a></li>');
            }
        }
        var delaySearch;
